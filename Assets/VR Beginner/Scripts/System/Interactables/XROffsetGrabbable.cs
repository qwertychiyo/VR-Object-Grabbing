...
public class XROffsetGrabbable : XRGrabInteractable
{
    ...
    protected override void OnSelectEntered(SelectEnterEventArgs evt)
    {
        ...
        if (interactor is XRDirectInteractor)
        {
            ...
            {
                OriginalPosition = interactor.attachTransform.localPosition,
                OriginalRotation = interactor.attachTransform.localRotation
            };
    ...
    {
        var interactor = evt.interactor;
        if (interactor is XRDirectInteractor && m_SavedTransforms.TryGetValue(interactor, out var savedTransform))
        {
    ...
    {
        // Only allow grabbing by direct interactor (prevents ray interactor grabs)
        if (!(interactor is XRDirectInteractor))
            return false;
